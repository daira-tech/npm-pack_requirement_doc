...
認証・サービス概要など、**mock に紐づかない自由記述**をカタログに載せられる。一覧は「ページ → 部品 → その他 → カラー」の順。
...
`main.ts` の `documents` 配列に追加する。詳細は `/docs/:name`、PDF は `/print/docs/:name`。
...
| その他を追加 | `documents/*/meta.ts` + `*.md` を作り、`main.ts` の `documents` に追加 |
